...
class Value:
    ...
    def realize_cached_data(self):
        ...
        self.cached_data = self.op.compute(
            *[x.realize_cached_data() for x in self.inputs] # recursion
        )
        self.cached_data
        return self.cached_data
...
class Tensor(Value):
    ...
    def backward(self, out_grad=None):
        out_grad = out_grad if out_grad \
                            else Tensor(numpy.ones(self.shape))
...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    # `output_tensor` is `self` upon each call.
    # `out_grad` is an identity matrix upon its first call.
    node_to_output_grads_list[output_tensor] = [out_grad]

    # Traverse graph in reverse topological order given the output_node that we are taking gradient w.r.t.
...
def find_topo_sort(node_list: List[Value]) -> List[Value]:
    """Given a list of nodes, return a topological sorted list of nodes ending in them.
