...
class NDArray:
    ...
    def _init(self, other):
        ...
        self._handle = other._handle # pointer to a 1D array

    @staticmethod
    def compact_strides(shape):
        """ Utility function to compute strides of a compact NDArray """
        stride = 1 # Stride of the last dimension must be 1.
        res = []
    ...
    def make(shape, strides=None, device=None, handle=None, offset=0):
        ...
        array._strides = NDArray.compact_strides(shape) if   strides is None \
                                                        else strides
        array._offset = offset
        array._device = device if   device is not None \
                               else default_device()
        #if handle is None:
        #    array._handle = array.device.Array(prod(shape))
        #else:
        #    array._handle = handle
        array._handle = handle if   handle is not None \
                               else array.device.Array(prod(shape))
        return array
    ...
    def permute(self, new_axes):
        ...
        Returns:
            NDarray : new NDArray object with permuted dimensions, pointing
                      to the same memory as the original NDArray (i.e., just
                      shape and strides changed).
        """

        ### BEGIN YOUR SOLUTION
        assert len(new_axes) == self.ndim,             'NDArray dimension mismatch'
        #assert set(new_axes) == set(range(self.ndim)), 'NDArray dimension mismatch'

        return self.as_strided(shape=tuple(self.shape[i] for i in new_axes),
    ...
    def broadcast_to(self, new_shape):
        ...
        Raises:
            assertion error if new_shape[i] != shape[i] for all i
                                              where shape[i] != 1
        Args:
    ...
    def __getitem__(self, idxs):
        ...
        array.  When passed notation such as a[1:5, :-1:2, 4, :], Python will
        ...
        if not isinstance(idxs, tuple):
            idxs = (idxs,)
        idxs = tuple(self.process_slice(s, i)
        ...
        ### BEGIN YOUR SOLUTION
        return self.as_strided(shape=tuple(len(range(idx.start, idx.stop, idx.step)) for idx in idxs),
                               strides=tuple(self.strides[i] * idx.step for i, idx in enumerate(idxs)),
                               offset=(self._offset +
                                       sum(self.strides[i] * idx.start for i, idx in enumerate(idxs))))
        ### END YOUR SOLUTION
