                                            ...
                                            uint32_t M, uint32_t N, uint32_t P)
{
    ...
    uint32_t xBlock = blockIdx.x,
             yBlock = blockIdx.y;
    // determine relative position
    uint32_t xTile = threadIdx.x,
             yTile = threadIdx.y;
    // determine absolute position
    uint32_t x = xBlock * BLOCK + xTile,
    ...
    for (uint32_t i = 0; i < (N + BLOCK-1) / BLOCK; i++)
    {
        ...
        __shared__ scalar_t aTile[BLOCK][BLOCK],
                            bTile[BLOCK][BLOCK];
        // pad tile with 0 if out of bound
        aTile[yTile][xTile] = i * BLOCK + xTile < N && y < M ? \
                              subA[yTile * N + xTile] : 0.0f;
        bTile[yTile][xTile] = x < P && i * BLOCK + yTile < N ? \
                              subB[yTile * P + xTile] : 0.0f;
        // make sure submatrices are loaded before computation
        __syncthreads();

        for (uint32_t j = 0; j < BLOCK; j++) {
            entry += aTile[yTile][j] * bTile[j][xTile];
        }
