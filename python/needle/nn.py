...
from math import prod # for computing size of tensor
...
class Embedding(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        """
        ### BEGIN YOUR SOLUTION
        one_hot = init.one_hot(n=self.num_embeddings,
                               i=x.reshape(shape=(prod(x.shape),)),
                               device=self.device,
                               dtype=self.dtype,
                               requires_grad=False)
        return ops.reshape(one_hot @ self.weight,
